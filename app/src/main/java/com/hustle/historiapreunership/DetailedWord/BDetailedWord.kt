...
class BDetailedWord : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        abjad = arrayListOf(
            getString(R.string.Belanda),
            getString(R.string.Belgia),
            ...
            override fun onItemClick(position: Int) {
                when (position) {
                    0 -> {
                        ...
                        intent.putExtra(SearchManager.QUERY,R.string.Belanda)
                        startActivity(intent)
                    }
                    1 -> {
                        val intent = Intent(Intent.ACTION_WEB_SEARCH)
                        intent.putExtra(SearchManager.QUERY,R.string.Belgia)
                        startActivity(intent)
                    }
                    2 -> {
                        val intent = Intent(Intent.ACTION_WEB_SEARCH)
                        intent.putExtra(SearchManager.QUERY,R.string.Brazil)
                        startActivity(intent)
                    }
                }
            }
