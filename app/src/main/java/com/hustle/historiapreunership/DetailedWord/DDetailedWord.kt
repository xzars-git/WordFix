...
class DDetailedWord : AppCompatActivity() {
            ...
            override fun onItemClick(position: Int) {
                when (position) {
                    0 -> {
                        ...
                        intent.putExtra(SearchManager.QUERY,R.string.Denmark)
                        startActivity(intent)
                    }
                    1 -> {
                        val intent = Intent(Intent.ACTION_WEB_SEARCH)
                        intent.putExtra(SearchManager.QUERY,R.string.Djibouti)
                        startActivity(intent)
                    }
                    2 -> {
                        val intent = Intent(Intent.ACTION_WEB_SEARCH)
                        intent.putExtra(SearchManager.QUERY,R.string.Dominika)
                        startActivity(intent)
                    }
                }
            }
